...
from shops 
INNER JOIN cats  on shops.id = cats.shops_id and cats.name = 'Murzik'
where exists(select * from shops s1 where s1.shopname = shops.shopname);
...
WHERE shops.id NOT in (
	SELECT s1.id 
		FROM shops as s1
        JOIN cats as c on s1.id=c.shops_id
